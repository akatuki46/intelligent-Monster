...
bool Renderer::show_another_window = false;
bool Renderer::show_hello_world = true;
bool Renderer::show_test_window = true;
...
void Renderer::Init()
{
	...
	swapChainDesc.BufferDesc.Width = SCREEN_RESOLUTION_WIDTH;
	swapChainDesc.BufferDesc.Height = SCREEN_RESOLUTION_HEIGHT;
	...
	float m_ImGuiFontSize = 15.0f;
	//	Fonts
	//  imgui/misc/fonts階層下にフォントが用意されている
	io.Fonts->AddFontFromFileTTF("imgui-master/misc/fonts/Roboto-Medium.ttf", m_ImGuiFontSize);
	...
	viewport.Width = (FLOAT)SCREEN_RESOLUTION_WIDTH;
	viewport.Height = (FLOAT)SCREEN_RESOLUTION_HEIGHT;
	...
	// ブレンドステート設定
	//このままだと半透明が描画できなくなる(AlphaToCoverageEnableがTRUEだから)
	//木の時にはTRUE　ほかの時はFALSEにすればいい
	...
	LIGHT light{};
	light.Enable = true;
	light.Direction = D3DXVECTOR4(0.0f, -1.0f, 0.0f, 0.0f);
...
//これと似たような感じでAlphaToCoverageEnableをTRUEにするのかFALSEにするのかを切り替えれば行けそう
...
void Renderer::SetWorldViewProjection2D()
{
	...
	D3DXMatrixOrthoOffCenterLH(&projection, 0.0f, (float)SCREEN_RESOLUTION_WIDTH, (float)SCREEN_RESOLUTION_HEIGHT, 0.0f, 0.0f, 1.0f);
...
{
	D3DXVECTOR4 hoge = D3DXVECTOR4(CameraPosition.x, CameraPosition.y, CameraPosition.z, 1.0f);
	m_DeviceContext->UpdateSubresource(m_CameraBuffer, 0, NULL, &hoge, 0, 0);
}
