...
void AnimetionModel::Load(const char* FileName)
{
...
void AnimetionModel::LoadAnimetion(const char* FileName, std::string AnimationName)
{
	m_Animetion[AnimationName] = aiImportFile(FileName, aiProcess_ConvertToLeftHanded);
	assert(m_Animetion[AnimationName]);
}

void AnimetionModel::CreateBone(aiNode* node)
{
	BONE bone;
...
void AnimetionModel::Unload()
{
	...
	for (auto pair : m_Animetion)
	{
		aiReleaseImport(pair.second);
	}
}

void AnimetionModel::Update(std::string AnimationName1, std::string AnimationName2, float BlendRate, int Frame)
{
	if (!m_Animetion[AnimationName1]->HasAnimations())
		return;

	if (!m_Animetion[AnimationName2]->HasAnimations())
		return;


	//アニメーションデータからボーンマトリクス算出
	aiAnimation* animation1 = m_Animetion[AnimationName1]->mAnimations[0];
	aiAnimation* animation2 = m_Animetion[AnimationName2]->mAnimations[0];
	...
	for (unsigned int c = 0; c < animation1->mNumChannels; c++)
	{
		...
		//BONE* bone2 = &m_Bone[nodeAnim2->mNodeName.C_Str()];
	...
	for (unsigned int m = 0; m < m_AiScene->mNumMeshes; m++)
	{
		...
		for (unsigned int v = 0; v < mesh->mNumVertices; v++)
		{
			...
			//ウェイトを考慮してマトリクス算出　
			// BoneWeightにはどっちの骨の動きから影響を受けるかの比率が入っている
			// （腕曲げたとき、2つの骨についている皮膚がどちらの骨の動きの影響を受けるか？という問題）
			//↓やりたいこと
...
void AnimetionModel::UpdateBoneMatrix(aiNode* node, aiMatrix4x4 matrix)
{
